...
error2:
	...
	ret = copy_to_user((void __user *)para.para_out_l, &ret, sizeof(ret));
	if (ret) {
...
static ssize_t hifi_debug_level_show(struct device *dev,
									  struct device_attribute *attr, char __user *buf)
{
	...
	return simple_read_from_buffer(buf, PAGE_SIZE, &pos, level_str, strlen(level_str));
}
										...
										const char __user *buf, size_t size)
{
	...
	g_misc_data.debug_level = hifi_get_debug_level_num(level_str[0]);
	return size;
}
										...
										const char __user *buf, size_t size)
{
	...
	g_misc_data.dsp_debug_level = hifi_get_debug_level_num(level_str[0]);
