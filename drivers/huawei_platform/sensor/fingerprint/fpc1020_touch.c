...
static void capture_post_processing(u8 *p_img)
{
    int x;
    int y;
    for (y = 0; y < NAV_IMG_H; y++) {
	for (x = 0; x < NAV_IMG_W; x++) {
		p_img[x+y*NAV_IMG_W] = p_img[(x*2)+y*(NAV_IMG_W*2)];
...
#ifdef FPC1020_NAV_DPAD_DBG
            pr_info("[FPC/DPAD] Navigation:active=%d, fpc1020->nav.sum_x:%d, fpc1020->nav.sum_y:%d,\n nav.threshold_key_start:%d, nav.threshold_key_offset=%d\n",
    ...
    switch (finger_status)
    {
        ...
        case FNGR_ST_MOVING:
            ...
            pr_info("[FPC/TOUCH] %s:touch moving sum x,y(%d, %d)/sum_x,sum_y(%d,%d)\n",
                    __func__, x, y, fpc1020->nav.nav_sum_x, fpc1020->nav.nav_sum_y);
#endif
           ...
           fpc1020->nav.enabled && (error >= 0))
    {
		...
		pr_info("[FPC] prepare report finger up\n");
            ...
            if (diffTime > 0)
            {
                ...
                {
#ifdef FPC1020_NAV_DBG
                    pr_info("[FPC]  get_movement move poll \n");
#endif
