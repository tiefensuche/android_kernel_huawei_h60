...
static ssize_t tpa2028_l_reg_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t size)
{
    int i = 0, ret;
    char temstr[MAX_BUFFER_LEN] = {0};
    char *reg, *regvalue, *str, *tmpstr;
    unsigned char w_buf[] = {0, 0};
    unsigned long val;	
 
    if(size >= MAX_BUFFER_LEN){
        loge("%s: input cmd is too long,aband it", __FUNCTION__);
        return size;
    }
    strncpy(temstr, buf, MAX_BUFFER_LEN - 1);
    str = temstr;

    if(temstr[0] == ' '){
        logi("%s: remove the front space char", __FUNCTION__);
		while(tmpstr = strsep(&str, " ")){
			if(*tmpstr == 0)
			  continue;
			else
			  break;
        ...
        if(!str){
            loge("%s: str get null", __FUNCTION__);
            return size;
        }
    } 
    ...
    reg = strsep(&str, ":"); 
    if(!reg){
        loge("%s: reg get null", __FUNCTION__);
        return size;
    }

    regvalue =str;
    if(!regvalue){
        loge("%s: regvalue get null", __FUNCTION__);
        return size;
    }

    ret =  strict_strtoul(reg, 16, &val);
    if(ret) {
       loge("%s: get reg error", __FUNCTION__);
       return size;
    }
    
    if(val < 1 || val > 7) {
       loge("%s: invalid reg 0x%02x", __FUNCTION__, val);
       return size;
    }
    
    w_buf[0] = (unsigned char)val;

    ret =  strict_strtoul(regvalue, 16, &val);
    if(ret) {
       loge("%s: get regvalue error", __FUNCTION__);
       return size;
    }

    if(val > 0xff) {
       loge("%s: invalid reg value 0x%02x", __FUNCTION__, val);
       return size;
    }

    w_buf[1] = (unsigned char)val;
    tpa2028_l_i2c_write(w_buf, 2);

    return size;
}


static DEVICE_ATTR(reg_ctrl, 0664, tpa2028_l_reg_show, tpa2028_l_reg_store);
