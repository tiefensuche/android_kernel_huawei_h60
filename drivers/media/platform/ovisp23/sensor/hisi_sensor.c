...
static bool hisi_sensor_power_on = false;
			...
			unsigned int cmd, void *arg)
{
	...
	if (!s_ctrl->sensor) {
		if(!isp_base){
			cam_err("%s isp_init must be done before sensor_init.", __func__);
			return -EBADF;
		}
	...
	if(!hisi_sensor_power_on  &&  (cdata->cfgtype != CFG_SENSOR_POWER_UP) && (cdata->cfgtype != CFG_SENSOR_GET_SENSOR_NAME) && (cdata->cfgtype != CFG_SENSOR_GET_PRODUCT_NAME)){
		cam_err("%s POWER_ON must be done before other CMD %d", __func__, cdata->cfgtype);
		return -EBADF;
	}
...
static int hisi_sensor_subdev_internal_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (s_ctrl->sensor == NULL || s_ctrl->sensor->func_tbl == NULL
		|| s_ctrl->csi_ctrl == NULL || s_ctrl->csi_ctrl->hisi_csi_disable == NULL || !hisi_sensor_power_on)
		return rc;
